...
1. **🎨 工业级现代化控制台 (Fanout PRO 2.0)**：
   - **三段式出网流量动态管线 (Visual Pipeline)**：直观展现「核心 443 入站 ➔ 动态穿透引擎 ➔ 真实出网节点」的全链路拓扑，实时流光扫掠指示；支持在管线控制条上一键挑选挂载出口或解绑恢复母机原生直连。
   - **纯白矢量极简美学**：彻底消除粗糙彩色表情符号，全面换装 24×24 极简矢量线条 SVG，支持悬停发光微投影（Glow Filter）。
   - **深色高对比 / 纯净科技白双主题引擎**：无缝平滑切换，支持 `localStorage` 记忆并内嵌防白屏闪烁（FOUC）优化。
   - **三栏网格架构出口卡片**：国别代码 + 真实落地 IP 快捷复制 + 住宅画像纯净度得分 + 挂载入站胶囊群 + SOCKS5 快速凭据直达。
   - **全端响应式系统**：完美适配桌面端、平板（双列看板）与手机移动端（竖屏自适应管线流动）。

2. **🌐 域名绑定、原生 SSL (HTTPS) 与 Caddy 自动化反代**：
   - 支持在 Web 设置与终端 CLI 菜单一键绑定自定义域名，所有节点分享链接与订阅源自动适配域名。
   - **原生 HTTPS**：支持挂载自定义证书（`.crt` / `.key`），支持证书到期时间与健康度实时检测。
   - **自动化反代**：集成 Caddy 自动化反向代理，母机 443 端口防冲突，免端口纯净访问面板。

3. **🌟 全节点大厅 (Node Explorer)**：
   - 界面顶栏直达「节点大厅」，一览所有可用的 VPN Gate 节点。
...
6. **📡 导出全部节点与客户端一键订阅源 (/sub)**：
   - 「导出链接」同时输出母机直连节点（如母机 443 直连）与全部出口挂载节点，不再遗漏直连。
...
- 管理界面原生支持防探测随机路径前缀、自定义域名绑定、原生 SSL (HTTPS) 证书热加载以及 Caddy 自动化反代，确保面板在公网环境下的通信安全。
